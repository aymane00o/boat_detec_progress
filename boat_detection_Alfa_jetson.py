...
INFER_SIZE       = 1280              # 640 = standard | 1280 = better for small/far boats
CONF_THRESHOLD   = 0.25             # lower = catch more boats (but more false positives)
IOU_THRESHOLD    = 0.40             # lower = less duplicate boxes
MIN_BOX_AREA     = 100              # ignore detections smaller than 20x20px (noise)
...
def detect_boats(model, frame):
    ...
    results = model(
        ...
        augment=True,         # multi-scale inference — better for far boats
    )
